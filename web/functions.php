...
function get_forecast($num, $i)
{
  $res = data_pull($num);
  $day = $res->weather->forecast[$i]->day_max_temp;
  $night = $res->weather->forecast[$i]->night_min_temp;
  $date = $res->weather->forecast[$i]->date;
  $array = array(
    "daytemp" => $day,
    "nighttemp" => $night,
    "date" => $date
...
function get_resort_status($num)
{
  $res = data_pull($num);
  $data = $res->run_report->resort_open;
  return $data;
}


function get_lifts_open($num)
{
  $res = data_pull($num);
  $data = $res->run_report->lifts_open;
  return $data;
}

function get_beginner_open($num)
{
  $res = data_pull($num);
  $data = $res->run_report->beginner_open;
  return $data;
}

function get_intermediate_open($num)
{
  $res = data_pull($num);
  $data = $res->run_report->intermediate_open;
  return $data;
}

function get_advanced_open($num)
{
  $res = data_pull($num);
  $data = $res->run_report->advanced_open;
  return $data;
}
